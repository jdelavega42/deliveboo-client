  ...
  methods: {
    ...
    addToCart(product, index) {
      ...
      if (existingProduct && this.quantity[index] > 0) {
        ...
        product.restaurant_id == this.store.state.cart[0].restaurant_id
      ) {
        this.store.state.cart.push({
          ...product,
          quantity: this.quantity[index],
        ...
        alert(
          "Puoi inserire prodotti di un solo ristorante, aggiungi altor di " +
  ...
  <div class="container-fluid">
    ...
    <div class="row row-cols-2">
      ...
      <div class="col restaurant-info restaurant-name">
        <div
          class="info d-flex flex-column justify-content-center align-items-center h-100"
        >
      ...
      <div class="col restaurant-info restaurant-image">
        <div
          class="image d-flex justify-content-center align-items-center h-100"
        >
          <h2>IMMAGINI</h2>
        </div>
      </div>
    ...
    <div class="menu d-flex flex-column align-items-center">
      <div
        class="rounded m-2 d-flex justify-content-around align-items-center"
      ...
      >
        ...
        <div
          class="cart-section d-flex flex-column h-100 justify-content-around h-100"
        >
          <div class="text-center" for="number">Quantità:</div>
          <div class="position-relative quantity-form">
            <div
              class="position-absolute error-quantity"
              v-if="quantity[index] < 1"
            >
            ...
            <div class="d-flex align-items-center h-100">
              <div
                class="p-2"
                @click="product.visibility ? decrement(index) : ''"
              >
                <i class="fa-solid fa-minus"></i>
              </div>
              <input
                :class="{ 'product-disabled': !product.visibility }"
                type="number"
                :id="'number-' + product.id"
                class="quantity-number form-control"
                name="number"
                v-model="quantity[index]"
                @input="addQuantity(index, $event.target.value)"
                min="1"
              />

              <div
                class="p-2"
                @click="product.visibility ? increment(index) : ''"
              >
